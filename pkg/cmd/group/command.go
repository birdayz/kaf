...
func newDeleteOffsetsCommand(a *app.App) *cobra.Command {
	var (
		topic         string
		partitionFlag int32
		allPartitions bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if offsetMap != "" {
				...
				err := json.Unmarshal([]byte(offsetMap), &partitionStrings)
				if err != nil {
					// Try parsing as map: {"0":null, "1":null}
					var partitionMap map[string]any
					err2 := json.Unmarshal([]byte(offsetMap), &partitionMap)
					if err2 != nil {
						return fmt.Errorf("invalid --offset-map format. Use JSON array like [\"0\", \"1\"] or map like {\"0\":null}")
					}
				...
				for _, pStr := range partitionStrings {
					...
					if err != nil {
						return fmt.Errorf("invalid partition ID in offset-map: %v", pStr)
					}
					partitions = append(partitions, int32(p))
				}
			} else if allPartitions {
				td, err := pkgtopic.Describe(ctx, cl.Admin, topic)
				if err != nil {
			...
			if len(partitions) == 0 {
				return fmt.Errorf("no partitions to delete offsets for")
			}
			...
			if g, ok := described[grp]; ok {
				if !slices.Contains([]string{"Empty", "Dead"}, g.State) {
					return fmt.Errorf("consumer group %s has active consumers in it, cannot delete offsets", grp)
				}
			}

			fmt.Fprintf(a.OutWriter, "Will delete offsets for group '%s' on topic '%s' for partitions: %v\n", grp, topic, partitions)
			...
			resps, err := cl.Admin.DeleteOffsets(ctx, grp, ts)
			if err != nil {
			...
			var failMsgs []string
			for _, p := range partitions {
				pErr, ok := resps.Lookup(topic, p)
				if !ok {
					failMsgs = append(failMsgs, fmt.Sprintf("  Partition %d: no response", p))
				} else if pErr != nil {
					failMsgs = append(failMsgs, fmt.Sprintf("  Partition %d: %v", p, pErr))
				} else {
			...
			if len(succeeded) > 0 {
				fmt.Fprintf(a.OutWriter, "Successfully deleted offsets for partitions: %v\n", succeeded)
			}
			if len(failMsgs) > 0 {
				fmt.Fprintf(a.ErrWriter, "Failed to delete offsets for the following partitions:\n")
				for _, msg := range failMsgs {
					fmt.Fprintln(a.ErrWriter, msg)
				}
				return fmt.Errorf("some offset deletions failed")
			}
			return nil
		},
	}
	...
	cmd.Flags().BoolVar(&allPartitions, "all-partitions", false, "delete offsets for all partitions")
	cmd.Flags().StringVar(&offsetMap, "offset-map", "", "delete offsets for specific partitions in JSON format, e.g. [\"0\", \"1\", \"2\"] or {\"0\":null, \"1\":null}")
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			offsets.Each(func(o kadm.OffsetResponse) {
				...
				start := max(committed-flagPeekBefore, 0)
