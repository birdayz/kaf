...
func NewCommand(a *app.App) *cobra.Command {
  ...
  kaf consume my-topic --header "env:prod"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if groupFlag != "" && cmd.Flags().Changed("offset") {
				fmt.Fprintf(a.ErrWriter, "WARNING: --offset is ignored when using --group (consumer group manages offsets)\n")
			}
			...
			if needsAdmin {
				...
				if tail > 0 {
					...
					partFilterSet := make(map[int32]bool, len(flagPartitions))
					for _, p := range flagPartitions {
						partFilterSet[p] = true
					}
					...
					endOffsets.Each(func(lo kadm.ListedOffset) {
						...
						if len(partFilterSet) > 0 && !partFilterSet[lo.Partition] {
							return
						}
			...
			maxOffsetSeen := make(map[int32]int64)
			...
			for {
				...
				fetches.EachRecord(func(rec *kgo.Record) {
					...
					if limitMessagesFlag > 0 && totalMessageCount >= limitMessagesFlag {
						limitReached = true
						return
					}
					a.HandleMessage(rec, &mu, outputFormat, headerFilter)
					totalMessageCount++
					if cur, ok := maxOffsetSeen[rec.Partition]; !ok || rec.Offset > cur {
						maxOffsetSeen[rec.Partition] = rec.Offset
					}
				})
