...
func Execute(version, commit string) error {
	...
	// Deprecated: --verbose was for sarama logging, no-op with franz-go.
	var verbose bool
	root.PersistentFlags().BoolVarP(&verbose, "verbose", "v", false, "Whether to turn on sarama logging")
	_ = root.PersistentFlags().MarkDeprecated("verbose", "no longer applicable (sarama removed)")
