...
func (c *Config) ActiveCluster() *Cluster {
	...
	for _, cluster := range c.Clusters {
		if cluster.Name == toSearch {
			// Return a copy to prevent modifications from leaking back
			// into the config's cluster list.
			cp := *cluster
			return &cp
		}
	}
...
func (c *Config) Write() error {
	...
	if err := encoder.Close(); err != nil {
		tmpFile.Close()
		os.Remove(tmpPath)
		return fmt.Errorf("flush config encoder: %w", err)
	}
