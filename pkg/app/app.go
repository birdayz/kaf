...
func (a *App) NewKafClient(opts ...kgo.Opt) (*kafclient.Client, error) {
	cl, warnings, err := kafclient.New(a.CurrentCluster, opts...)
	if err != nil {
		return nil, fmt.Errorf("unable to create client: %w", err)
	}
	for _, w := range warnings {
		fmt.Fprintf(a.ErrWriter, "WARNING: %s\n", w)
	}
	return cl, nil
}
...
func (a *App) NewSchemaCache() (*avro.SchemaCache, error) {
	...
	cache, warnings, err := avro.NewSchemaCache(a.CurrentCluster.SchemaRegistryURL, username, password)
	if err != nil {
...
// SetupProtoDescriptorRegistry loads proto files if --proto-type or --key-proto-type is set.
func (a *App) SetupProtoDescriptorRegistry(cmd *cobra.Command, args []string) error {
	if a.ProtoType != "" || a.KeyProtoType != "" {
